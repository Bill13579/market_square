...
// Tests the ReservationIter (safe publishing) path.
...
fn benchmark_market_square() {
    println!("--- Market Square (Broadcast, ReservationIter) ---");
            ...
            for msg_idx in 0..(MESSAGES_PER_WRITER / BATCH_SIZE) {
                loop {
                    ...
                    match writer.reserve::<WriterIsExclusive>(BATCH_SIZE) {
                        Ok(reservation) => {
                            // Use the safe ReservationIter path instead of unsafe publish
                            let mut iter = reservation.into_iter();
                            let mut i = 0;
                            for mut slot in &mut iter {
                                slot.write((msg_idx * BATCH_SIZE + i) as u64);
                                i += 1;
                            }
                            if N_WRITERS == 1 {
                                match iter.publish::<market_square::area::Exclusive>() {
                                    Ok(_) => (),
